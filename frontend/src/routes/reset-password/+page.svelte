<script lang="ts">
	...
	import { authError, authLoading, resetPassword } from '$lib/stores/auth';
	...
	const handleSubmit = async (event: Event) => {
		...
		if (success) await goto('/login?reset=1');
	};
</script>
...
<div class="mx-auto max-w-md">
	...
	{#if !token}
		<p class="mt-6 text-sm text-red-600">Lien invalide ou manquant. Demandez un nouveau lien depuis la page « Mot de passe oublié ».</p>
		<p class="mt-4">
			<a href="/forgot-password" class="font-medium text-amber-700 hover:underline">Mot de passe oublié</a>
		</p>
	{:else}
		<form class="mt-6 space-y-4" onsubmit={handleSubmit}>
			<div>
				<label class="text-sm font-medium text-slate-700" for="reset-password">Nouveau mot de passe</label>
			...
			<div>
				<label class="text-sm font-medium text-slate-700" for="reset-password-confirm">Confirmer le mot de passe</label>
				<input
			...
			>
				{#if $authLoading}Enregistrement...{:else}Enregistrer le mot de passe{/if}
			</button>
		</form>
